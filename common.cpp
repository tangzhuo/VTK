...
double sprod(DOC *a, DOC *b);
double sprod_ss(WORD *a, WORD *b);
double sprod_i(DOC *a, DOC *b, int i, int j);
		    ...
		    long int *totwords, long int *totdoc, KERNEL_PARM *kernel_parm)
{
  ...
  while((!feof(docfl)) && fgets(line,(int)ll,docfl)) {
	...
	docs[dnum].k_score = doc.k_score;
    //printf("docs[%d].k_score = %lf",dnum,docs[dnum].k_score);
...
//  printf("\nline = : %s\n",line);
  ...
  PARAM_VECT = kernel_parm->tree_kernel_param; // restore parameter file flag
...
//  printf("LINE:%s\n",line+pos);fflush(stdout);
  ...
  /*while((!isspace((int)line[pos])) && line[pos]) pos++;*/
   if(line[pos]!=0) read_vector_set(line+pos, doc, max_words_doc, totwords);// read the vector set 
