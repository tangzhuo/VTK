...
/*Julie's test of production
	  printf("\n\n");
	  for(te = 0; te<forest_vec_tmp[tree_index]->listSize; te++)
		  printf("production[%d]: %s   node:%s\n",te,forest_vec_tmp[tree_index]->orderedNodeSet[te].node->sName,forest_vec_tmp[tree_index]->orderedNodeSet[te].sName);
...
void fillTheNodeList(TreeNode *N, char * parentname, OrderedTreeNode *list, int *counter){
	...
	*production=0;
	if((N->iNoOfChildren>0) /*&& (N)*/){
       if(TKGENERALITY==PARTIAL_TREE_KERNEL || TKGENERALITY == NOLEAVES_PT_KERNEL)
		   sprintf(production,"%s",N->sName); // only PT like kernels work on nodes 
       else 
		   sprintf(production,"%s->",N->sName);             // the other kernels works on productions
...
/* it considers also the non terminals */
	...
	else if(TKGENERALITY == SUBSET_TREE_KERNEL){//修改STK树核，叶子结点写入
		 list[*counter].node=N;
	     N->production=list[*counter].sName=strdup(N->sName);
	     N->pre_terminal=-1; // these nodes are not associated with a standard production 
	     (*counter)++;        // so they are terminal (less then pre-terminal)
...
int i=0;
	...
	/*TEST
	for(;i<tree->root->pChild[0]->iNoOfChildren;i++)
		printf("\ntchildren[%d] = %s\n",i,tree->root->pChild[0]->pChild[i]->sName);
	*/
	...
	/* printf("\n\nNormal PRINTing\n");
    for(i=0;i<counter;i++)
		printf("\nprod[%d]:%s ID:%d ,pre_ter = %d ",i+1,tree->orderedNodeSet[i].sName, tree->orderedNodeSet[i].node->nodeID,tree->orderedNodeSet[i].node->pre_terminal);
